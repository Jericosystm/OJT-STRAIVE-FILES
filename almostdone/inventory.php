...
<body>
    ...
    const remarksDiv = document.getElementById('remarksWrapper'); // New

    // 1. Logic for Onsite
    onsiteDiv.style.display = (loc === 'Onsite') ? 'block' : 'none';

    // 2. Logic for Release/Return (Shows Agent AND Remarks)
    if (loc === 'Release' || loc === 'Return') {
        ...
        document.getElementById('remarks').value = ''; // Clear if hidden
    }
    ...
    async function editAsset(item) {
        ...
        setVal('switch_port', item.switch_port); // This will now get the REAL port from inventory_items
        setVal('department', item.department);
    ...
    document.getElementById('assetForm').onsubmit = async (e) => {
        ...
        const data = {
            ...
            remarks: getVal('remarks') // <--- Add this line
        };
