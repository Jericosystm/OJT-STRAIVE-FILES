...
try {
  ...
  if ($action === 'create') {
    ...
    if ($stmt->execute()) {
        ...
        // 3. SYNC MOVEMENT & EMAIL (RETURN/RELEASE ONLY)
...
recordActivity($conn, $_SESSION['user_id'] ?? null, "UPDATE_ASSET", $logDetails);
            ...
            // --- SYNC MOVEMENT & EMAIL (RETURN/RELEASE ONLY) ---
            if (in_array($location, ['Release', 'Return'])) {
    ...
    // 1. Get the name before deleting so we can put it in the log
...
function syncMovement($conn, $asset_id, $data, $operator) {
    ...
    $remarks = $data['remarks'] ?? ''; // Capture remarks
...
$log->bind_param("issssssssssss", $asset_id, $data['asset_name'], $data['hostname'], $data['serial_num'], $data['device_type'], $loc, $rel, $ret, $operator, $data['agent_name'], $data['agent_email'], $data['immediate_supmail'], $remarks);    $log->execute();
}
...
function recordActivity($conn, $userId, $action, $details) {
    ...
    // Use the session user_id if provided, otherwise default to a system ID or 0
    $uId = $userId ? $userId : 0; 
