...
$page_title = "MIAMI DEPARTMENT";
$back_link = "prod_map.php"; 

$department_name = "Miami"; 
$total_seats = 32; 
...
$stmt = $conn->prepare("SELECT * FROM production_floor_map WHERE department = ? ORDER BY id DESC LIMIT ?");
...
if ($result->num_rows === 0) {
    echo "<div style='color: white; background: #ef4444; padding: 10px; text-align: center; font-size: 0.8rem;'>";
    echo "Notice: No data found for '" . htmlspecialchars($department_name) . "' department.";
    echo "</div>";
}
...
<head>
    ...
    <style>
        ...
        /* Support for Dark Mode Theme Sync */
        [data-theme="dark"] {
            --bg: #121212;
            --card-bg: #1e1e1e;
            --text-dark: #f1f5f9;
        ...
        .container { height: calc(100vh - 72px); padding: 1.5rem 2rem; display: flex; flex-direction: column; box-sizing: border-box; max-width: 1600px; margin: 0 auto; }

        .header-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; }
        .header-row h1 { font-weight: 800; font-size: 1.8rem; letter-spacing: -0.03em; margin: 0; }

        #searchInput { width: 320px; padding: 0.8rem 1rem 0.8rem 2.5rem; border-radius: 14px; border: 1px solid var(--border); background: var(--card-bg); color: var(--text-dark); font-size: 0.9rem; transition: all 0.3s; outline: none; box-shadow: var(--shadow-soft); }
        #searchInput:focus { border-color: var(--primary); width: 350px; }

        .map-grid-container { background: var(--card-bg); padding: 2rem; border-radius: 30px; border: 1px solid var(--border); flex-grow: 1; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-soft); overflow: hidden; }

        .map-grid { display: grid; grid-template-columns: repeat(8, 1fr); grid-template-rows: repeat(4, 1fr 20px); gap: 12px; width: 100%; height: 100%; }

        .walkway { grid-column: span 8; background: transparent; pointer-events: none; }

        .seat-box { border-radius: 14px; text-align: center; border: 1px solid var(--border); background: var(--card-bg); transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; box-shadow: var(--shadow-soft); padding: 5px; }
        ...
        .drag-over { background: var(--primary-light) !important; transform: scale(1.1) !important; border: 2px solid var(--primary) !important; }
        ...
        .status-legend { display: flex; justify-content: center; gap: 30px; margin-top: 1.5rem; background: var(--card-bg); padding: 0.8rem 2rem; border-radius: 50px; width: fit-content; margin-left: auto; margin-right: auto; box-shadow: var(--shadow-soft); border: 1px solid var(--border); }
        ...
        .modal-content { background: var(--card-bg); color: var(--text-dark); width: 420px; padding: 2.5rem; border-radius: 32px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); border: 1px solid var(--border); }
    </style>
</head>
<body id="body">
...
<div class="container">
    <div class="header-row">
        <div><h1>Miami Floor Plan</h1></div>
    ...
    <div class="map-grid-container">
        <div class="map-grid" id="mapGrid">
            ...
            for($i = 0; $i < $total_seats; $i++): 
                $row = $stations[$i] ?? null;
                $id = $row['id'] ?? ($total_seats - $i); 
                $cubicle_num = $total_seats - $i;
                $cubicle_name = "MIA-" . str_pad($cubicle_num, 4, '0', STR_PAD_LEFT);
    ...
    <div class="status-legend">
        ...
        <div class="legend-item" style="border-left: 1px solid var(--border); padding-left: 20px;">Total: <?php echo $total_seats; ?></div>
    </div>
</div>
...
<script>
    ...
    function performSwap(src, tgt) {
        ...
        fetch('miami.php', { method: 'POST', body: formData })
