...
$page_title = "SACRAMENTO LIVE MAP";
...
<head>
    ...
    <style>
        ...
        [data-theme='dark'] {
            ...
            --text-muted: #94a3b8;
            --border: #334155;
            --occupied-bg: rgba(34, 197, 94, 0.1);
            --occupied-border: #22c55e;
            --vacant-hover-bg: rgba(148, 163, 184, 0.1);
            --vacant-hover-border: #64748b;
            --table-surface: #334155;
            --table-shadow: #0f172a;
        }

        body, html { margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg); height: 100vh; overflow: hidden; transition: background 0.3s ease; }
        ...
        .floor-plan-wrapper { 
            ...
            display: grid; 
            grid-template-columns: 80px repeat(8, var(--seat-w)); 
            grid-template-rows: auto auto 4vh auto; 
            gap: var(--gap); 
            width: fit-content; 
            border: 1px solid var(--border);
            transform: rotateX(4deg); /* Subtle tilt */
        }
        ...
        .table-surface {
            ...
            padding: 12px;
            border-radius: 12px;
            box-shadow: 0 8px 0 0 var(--table-shadow);
        ...
        .horizontal-table {
            ...
            padding: 15px;
            border-radius: 15px;
            box-shadow: 0 10px 0 0 var(--table-shadow);
            display: flex;
            gap: var(--gap);
            margin-top: 10px;
        }

        /* Rest of the styles preserved exactly */
        .edit-sidebar { position: fixed; right: 20px; top: 12vh; background: var(--card-bg); padding: 20px; border-radius: 15px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); border: 1px solid var(--border); z-index: 100; text-align: center; width: 120px; color: var(--text-dark); }
        ...
        #searchInput { width: 300px; padding: 10px 15px 10px 35px; border-radius: 12px; border: 1px solid var(--border); outline: none; background: var(--card-bg); color: var(--text-dark); font-family: inherit; }
        ...
        .drag-over { background: #fff7ed !important; transform: scale(1.1) rotate(2deg) !important; border: 2px solid var(--primary-orange) !important; }
        ...
        .seat-box { background: var(--card-bg); border: 1.5px solid var(--border); border-radius: 8px; padding: 0.5vh; height: var(--seat-h); width: var(--seat-w); display: flex; flex-direction: column; justify-content: center; text-align: center; cursor: pointer; transition: 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); box-sizing: border-box; color: var(--text-dark); position: relative; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
        .seat-box:hover { transform: translateY(-12px) scale(1.05); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.2); border-color: var(--primary-orange); z-index: 10; }
        ...
        .port-main-box { background: var(--bg); border: 2px dashed var(--border); border-radius: 12px; height: 7vh; display: flex; flex-direction: column; align-items: center; justify-content: center; font-weight: 800; color: var(--text-muted); cursor: pointer; font-size: 0.6rem; }
        .port-popup { visibility: hidden; opacity: 0; position: absolute; left: 110%; top: 50%; transform: translateY(-50%); background: var(--card-bg); border-radius: 15px; padding: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); z-index: 100; width: 220px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; border: 1px solid var(--border); transition: 0.2s; }
        .port-trigger-container:hover .port-popup { visibility: visible; opacity: 1; }
        .port-item { background: var(--bg); border: 1px solid var(--border); border-radius: 10px; padding: 6px; display: flex; flex-direction: column; align-items: center; cursor: pointer; transition: 0.2s; color: var(--text-dark); }
        ...
        .status-bar { margin-top: 3vh; display: flex; justify-content: center; gap: 30px; background: var(--card-bg); padding: 10px 40px; border-radius: 50px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); border: 1px solid var(--border); color: var(--text-dark); z-index: 100; }
...
<div class="main-container">
    <div class="floor-plan-wrapper">
        ...
        <div class="managers-container">
            <div class="table-surface" style="padding: 15px; border-radius: 15px;">
                <div class="managers-row-flex">
                    ...
                    <div class="cell-tv-group" style="display:flex; flex-direction:column; width:var(--seat-w);">
                        <div class="samsung-tv-box">
                            <i class="fa-solid fa-display" style="font-size: 0.7rem;"></i> TV
                        </div>
