...
if (isset($_GET['delete'])) {
    ...
    // Fetch username for the log before deleting
    $stmt_name = $conn->prepare("SELECT username FROM users WHERE id = ?");
    $stmt_name->bind_param("i", $id);
    $stmt_name->execute();
    $target_user = $stmt_name->get_result()->fetch_assoc()['username'] ?? "Unknown ID: $id";
    ...
    if ($stmt->execute() && $stmt->affected_rows > 0) {
        // --- TRACKER LOGIC ---
        $log_stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, created_at) VALUES (?, 'DELETE', ?, NOW())");
        $details = "Permanently removed user access for: " . $target_user;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create_user'])) {
    ...
    if ($stmt->execute()) {
        // --- TRACKER LOGIC ---
        $log_stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, created_at) VALUES (?, 'INSERT', ?, NOW())");
        $details = "Granted access to new user: $user with role: " . strtoupper(str_replace('euc_', '', $role));
        $log_stmt->bind_param("is", $_SESSION['user_id'], $details);
        $log_stmt->execute();
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_user'])) {
    ...
    if ($stmt->execute()) {
        // --- TRACKER LOGIC ---
        $log_stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, created_at) VALUES (?, 'UPDATE', ?, NOW())");
        $details = "Reconfigured system identity for user: $user (Role: $role)";
        $log_stmt->bind_param("is", $_SESSION['user_id'], $details);
        $log_stmt->execute();
    }
