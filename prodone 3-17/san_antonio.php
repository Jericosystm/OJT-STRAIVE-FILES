...
// Calculate vacant if rows are missing from DB
...
<div class="container">
    ...
    <?php 
    // Create a lookup map by cubicle index to make sure we place data in the right spot
    $station_map = [];
    foreach($stations as $s) {
        // Assuming your 'hostname' or another field doesn't tell us the position,
        // we'll rely on the order they came from the DB for the first 49.
    }

    for($i = 0; $i < 49; $i++): 
        $row = $stations[$i] ?? null;
        
        // CRITICAL FIX: If row exists, use its DB ID. 
        // If not, we use 0 or a flag to tell the update handler to INSERT instead of UPDATE.
        $db_id = isset($row['id']) ? $row['id'] : "new_" . ($i + 1); 
        
        $cubicle_num = $i + 1;
$cubicle_name = "SA - " . str_pad($cubicle_num, 4, '0', STR_PAD_LEFT);
    ...
    ?>
        <div class="seat-box <?php echo $status; ?>" 
             id="seat-<?php echo $db_id; ?>"
             data-id="<?php echo $db_id; ?>"
             data-hostname="<?php echo strtolower($hostname); ?>"
             title="<?php echo $tooltip; ?>"
             onclick="handleSeatClick(event, '<?php echo $db_id; ?>', '<?php echo $cubicle_name; ?>', '<?php echo addslashes($hostname); ?>', '<?php echo addslashes($switch_port); ?>', '<?php echo $status; ?>')">
        ...
        <?php 
        // Walkway logic: 7 columns * 2 rows = 14 seats per block
