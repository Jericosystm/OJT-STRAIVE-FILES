...
/**
 * Helper function to sync hostname location to inventory
 */
function syncInventory($conn, $hostname, $cubicle, $dept, $status) {
    if (empty($hostname)) return;

    if ($status === 'Vacant') {
        // Clear the inventory record for this specific hostname
        $stmt = $conn->prepare("UPDATE inventory_items SET cubicle_number = '', location = 'WFH' WHERE hostname = ?");
        $stmt->bind_param("s", $hostname);
    } else {
        // First: Clear anyone ELSE who might have been assigned to this cubicle (to avoid duplicates)
        $clearOld = $conn->prepare("UPDATE inventory_items SET cubicle_number = '', location = 'WFH' WHERE cubicle_number = ? AND hostname != ?");
        $clearOld->bind_param("ss", $cubicle, $hostname);
        $clearOld->execute();

        // Second: Assign the new hostname to this cubicle
        $stmt = $conn->prepare("UPDATE inventory_items SET cubicle_number = ?, department = ?, location = 'Onsite' WHERE hostname = ?");
...
if(isset($_POST['swap_seats'])) {
    ...
    if($updateSource->execute() && $updateTarget->execute()) {
        $sourceCubicle = "SAS-" . str_pad($sourceId, 4, '0', STR_PAD_LEFT);
        $targetCubicle = "SAS-" . str_pad($targetId, 4, '0', STR_PAD_LEFT);

        // Sync both items to their new cubicles in Inventory
...
if(isset($_POST['update_seat'])) {
    ...
    $status = $_POST['status']; 
    $cubicle_name = "SAS-" . str_pad($id, 4, '0', STR_PAD_LEFT);
    ...
    if($stmt->execute()) {
        if($status === 'Vacant') {
            // Specifically clear the cubicle link for whoever was here
            $clear = $conn->prepare("UPDATE inventory_items SET cubicle_number = '', location = 'WFH' WHERE cubicle_number = ?");
            $clear->bind_param("s", $cubicle_name);
            $clear->execute();
        } else {
...
// ... logic for fetching $stations and HTML follows ...
...
<head>
    ...
    <style>
        ...
        .map-grid { 
            ...
            grid-template-rows: 1fr 1fr 25px 1fr 1fr 25px 1fr 1fr 25px 1fr;
