...
$cubicles = [];
$cubicles_result = $conn->query("SELECT DISTINCT cubicle_no FROM production_floor_map WHERE status = 'Vacant' ORDER BY cubicle_no ASC");
if ($cubicles_result) {
...
if ($target_status === 'All') {
    if (!empty($search_query)) {
        $sql = "SELECT * FROM inventory_items WHERE (asset_name LIKE ? OR hostname LIKE ? OR serial_num LIKE ? OR cubicle_number LIKE ?) ORDER BY updated_at DESC";
    ...
    if (!empty($search_query)) {
        $sql = "SELECT * FROM inventory_items WHERE status = ? AND (asset_name LIKE ? OR hostname LIKE ? OR serial_num LIKE ? OR cubicle_number LIKE ?) ORDER BY updated_at DESC";
        $like_param = "%$search_query%";
...
<body>
    ...
    try {
        // Force lowercase filename to match standard naming
        ...
        const result = await res.json(); // Use .json() directly
        ...
        alert("System Error: Could not connect to inventory_crud.php. Check console (F12).");
    }
}
