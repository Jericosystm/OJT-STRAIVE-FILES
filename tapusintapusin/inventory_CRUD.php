...
if (isset($_SESSION['username'])) {
    ...
    // --- UPDATED EXTRACTION FOR RELEASE/RETURN ---
    if (in_array($location, ['Release', 'Return', 'WFH'])) {
        ...
        $user_email  = trim($data['user_email'] ?? ''); // New: Logged in user email
        $accessories = trim($data['accessories'] ?? ''); // New: Accessories list
    } else {
    ...
    // We pass $registered_user_email here
    ...
    if (!$is_inactive && $cubicle_id && strcasecmp($location, 'Onsite') == 0) {
        // Check if the cubicle is occupied by SOMEONE ELSE (not the current hostname)
...
// --- UPDATED LOGISTICS SYNC HELPER ---
// --- FIXED LOGISTICS SYNC HELPER ---
function syncMovement($conn, $asset_id, $data, $operator, $reg_email) {
    ...
    // Note: $reg_email is used for the 'user_email' column
    $log->bind_param("issssssssssssss", $asset_id, $data['asset_name'], $data['hostname'], $data['serial_num'], $data['device_type'], $loc, $rel, $ret, $operator, $data['agent_name'], $data['agent_email'], $reg_email, $data['immediate_supmail'], $remarks, $accessories);    
    $log->execute();
}
// --- UPDATED NOTIFY MOVEMENT (EMAIL) ---
