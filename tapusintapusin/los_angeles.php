...
$page_title = "LOS ANGELES DEPARTMENT";
$back_link = "prod_map.php";

$department_name = "Los Angeles"; 
$total_seats = 63; // 9 columns x 7 rows
...
<head>
    ...
    <style>
        ...
        .container { height: calc(100vh - 72px); padding: 1rem 2rem; display: flex; flex-direction: column; box-sizing: border-box; max-width: 1600px; margin: 0 auto; }
        .header-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-shrink: 0; }
        .header-row h1 { font-weight: 800; font-size: 1.8rem; letter-spacing: -0.03em; margin: 0; background: linear-gradient(to right, var(--text-dark), var(--primary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .map-grid-container { background: var(--card-bg); padding: 1.5rem; border-radius: 30px; border: 1px solid var(--border); flex-grow: 1; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-soft); overflow: auto; margin-bottom: 1rem; }
        
        .map-grid { display: grid; grid-template-columns: repeat(9, 1fr); gap: 10px; width: 100%; height: 100%; max-height: 800px; }

        .seat-box { border-radius: 12px; text-align: center; border: 1px solid transparent; background: var(--card-bg); color: var(--text-dark); transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; box-shadow: var(--shadow-soft); padding: 5px; }
        ...
        .edit-sidebar { position: fixed; right: 20px; top: 85px; background: var(--card-bg); color: var(--text-dark); padding: 20px; border-radius: 20px; box-shadow: var(--shadow-hover); border: 1px solid var(--border); z-index: 100; display: flex; flex-direction: column; gap: 10px; }
        ...
        .status-legend { display: flex; justify-content: center; gap: 30px; background: var(--card-bg); color: var(--text-dark); padding: 0.8rem 2rem; border-radius: 50px; width: fit-content; margin: 0 auto; box-shadow: var(--shadow-soft); border: 1px solid var(--border); flex-shrink: 0; }
...
<div class="container">
    <div class="header-row">
        <div><h1>Los Angeles Floor Plan</h1></div>
    ...
    <div class="map-grid-container">
        <div class="map-grid" id="mapGrid">
            ...
            for($i = 0; $i < $total_seats; $i++): 
                ...
                $id = $row['id'] ?? ($i + 1); 
                $cubicle_num = $i + 1;
                $cubicle_name = "LAL-" . str_pad($cubicle_num, 4, '0', STR_PAD_LEFT);
...
<script>
    ...
    window.onload = function() {
        const savedMode = localStorage.getItem('swapModeEnabled_LA');
        if (savedMode === 'true') {
    ...
    function toggleEditMode() {
        ...
        localStorage.setItem('swapModeEnabled_LA', isEditMode);
