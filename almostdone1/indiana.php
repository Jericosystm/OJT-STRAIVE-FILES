...
if(isset($_POST['swap_seats'])) {
    ...
    try {
        ...
        if (!empty($srcHost)) {
            // Move Source to Target's Cubicle with TEMP Hostname
        ...
        if (!empty($tgtHost)) {
            // Move Target to Source's Cubicle
        ...
        if (!empty($srcHost)) {
            // Restore Source's original hostname
            $tmpName = $srcHost . $tempHostSuffix;
...
if(isset($_POST['update_seat'])) {
    ...
    // Fixed: Updated column name to switch_port to match the database schema
