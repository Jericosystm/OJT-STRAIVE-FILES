...
$cols = "`id`, `asset_name`, `host_name`, `serial_num`, `location`, `department`, `cubicle_number`, `device_type`, `status`, `updated_at`, `created_at`, `remarks`";
...
$sql = "SELECT $cols FROM inventory_items WHERE (`asset_name` LIKE ? OR `host_name` LIKE ? OR `serial_num` LIKE ? OR `cubicle_number` LIKE ?) ORDER BY updated_at DESC";
...
$sql = "SELECT $cols FROM inventory_items ORDER BY updated_at DESC";
...
$sql = "SELECT $cols FROM inventory_items WHERE `status` = ? AND (`asset_name` LIKE ? OR `host_name` LIKE ? OR `serial_num` LIKE ? OR `cubicle_number` LIKE ?) ORDER BY updated_at DESC";
...
$sql = "SELECT $cols FROM inventory_items WHERE `status` = ? ORDER BY updated_at DESC";
...
<div class="detail-item"><i class="fa-solid fa-desktop"></i><span><?php echo htmlspecialchars($item['cubicle_number'] ?? 'N/A'); ?></span></div>
