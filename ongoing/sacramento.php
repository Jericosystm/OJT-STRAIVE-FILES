...
if(isset($_POST['swap_seats'])) {
    ...
    $stmt = $conn->prepare("SELECT hostname, status, department FROM production_floor_map WHERE id = ?");
    ...
    $updateSource = $conn->prepare("UPDATE production_floor_map SET hostname=?, status=?, department=? WHERE id=?");
    $updateSource->bind_param("sssi", $targetData['hostname'], $targetData['status'], $targetData['department'], $sourceId);
    
    $updateTarget = $conn->prepare("UPDATE production_floor_map SET hostname=?, status=?, department=? WHERE id=?");
    $updateTarget->bind_param("sssi", $sourceData['hostname'], $sourceData['status'], $sourceData['department'], $targetId);
...
if(isset($_POST['update_seat'])) {
    ...
    $stmt = $conn->prepare("UPDATE production_floor_map SET hostname=?, status=?, department=? WHERE id=?");
...
function renderSeat($index, $stations) {
    $row = null;
    $searchLabel = "SAC-" . str_pad($index, 4, '0', STR_PAD_LEFT);
    
    foreach($stations as $s) {
        if (($s['hostname'] ?? '') == $searchLabel || (isset($s['display_index']) && $s['display_index'] == $index)) {
            $row = $s;
            break;
        }
    }
    ...
    if($isPortOnly) {
        $html = "<div class='port-item' title='$tooltip' data-id='$id' data-hostname='".strtolower($hostname)."' onclick=\"handleSeatClick(event, '$id', '$label', '$hostname', '$port', '$status', true)\">";
        $html .= "<i class='fa-solid fa-plug-circle-bolt'></i>";
        $html .= "<span>$label</span>";
        $html .= "</div>";
    } else {
        $html = "<div class='seat-box $activeClass' data-id='$id' data-hostname='".strtolower($hostname)."' title='$tooltip' onclick=\"handleSeatClick(event, '$id', '$label', '$hostname', '$port', '$status', false)\">";
        $html .= "<div class='seat-label'>$label</div>";
        $html .= "<div class='seat-host'>".($hostname ?: 'Available')."</div>";
        $html .= "<div class='seat-port'>$port</div>";
        $html .= "</div>";
    }
    return $html;
}
    ...
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <div style="display: flex; gap: 10px; margin-top: 20px;">
                <button type="submit" name="update_seat" style="flex:2; background: var(--primary); color: white; border: none; padding: 12px; border-radius: 12px; font-weight: 700; cursor: pointer;">Save</button>
