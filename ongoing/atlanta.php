...
function syncInventory($conn, $hostname, $cubicle, $dept, $status) {
    ...
    $cubicle  = strtoupper(trim($cubicle)); // e.g., "ATL-0001"
    $dept     = trim($dept);
    $location = ($status === 'Occupied' && $hostname !== 'N/A') ? 'Onsite' : 'N/A';

    // 2. The "Aggressive" Update
    // We use UPPER() on the column name to force a match even if the DB has lowercase
    $stmt = $conn->prepare("UPDATE inventory_items SET hostname = ?, department = ?, location = ? WHERE UPPER(trim(cubicle_number)) = ?");
    $stmt->bind_param("ssss", $hostname, $dept, $location, $cubicle);
    $stmt->execute();

    // 3. LOG THE RESULT (Check your XAMPP/Server error log!)
    if ($stmt->affected_rows > 0) {
        error_log("SUCCESS: Cubicle $cubicle updated to $hostname");
    } else {
        error_log("FAILED: No row found for cubicle '$cubicle' in inventory_items");
    }
}
...
if(isset($_POST['swap_seats'])) {
    ...
    if($updateSource->execute() && $updateTarget->execute()) {
        $sourceCubicle = "ATL-" . str_pad($sourceId, 4, '0', STR_PAD_LEFT);
        $targetCubicle = "ATL-" . str_pad($targetId, 4, '0', STR_PAD_LEFT);

        // Sync both assets to their new cubicle positions
...
if(isset($_POST['update_seat'])) {
    ...
    // 1. Get OLD hostname to clear it if it's being replaced
    ...
    if($stmt->execute()) {
        // 3. Clear the old host in inventory if it's being moved/removed
        if (!empty($oldHost) && $oldHost !== $hostname) {
            syncInventory($conn, 'N/A', $cubicle_name, $department_name, 'Vacant');
        }

        // 4. Sync the new host to the inventory
        syncInventory($conn, $hostname, $cubicle_name, $department_name, $status);

        // 5. STOP HERE SO WE CAN SEE IF IT WORKED
        die("<div style='padding:50px; background:#fff; border:5px solid #22c55e; font-family:sans-serif;'>
                <h1 style='color:#15803d;'>✅ DEBUG MODE ACTIVE</h1>
                <p>Floor Map updated for <b>$cubicle_name</b>.</p>
                <p>Sync function was just sent: <b>Host: $hostname</b> | <b>Status: $status</b></p>
                <hr>
                <p><strong>Next Step:</strong> Open your <code>inventory_items</code> table in phpMyAdmin.</p>
                <p>Look for cubicle <b>$cubicle_name</b>. Does the hostname now show <b>$hostname</b>?</p>
                <a href='atlanta.php' style='color:blue;'>Click here to go back after checking</a>
             </div>");
    }
}
...
<div id="modalOverlay" class="modal-overlay">
    <div class="modal-content">
        ...
        <form method="POST" action="atlanta.php">
