...
function renderSeat($index, $stations) {
    ...
    foreach($stations as $s) {
        if (($s['hostname'] ?? '') == $label || (isset($s['display_index']) && $s['display_index'] == $index)) {
            $row = $s;
            break;
        }
    }
    
    if (!$row && isset($stations[$index-1])) { $row = $stations[$index-1]; }
    ...
    return "<div class='seat-box $activeClass' data-id='$id' data-hostname='".strtolower($hostname)."' title='$tooltip' onclick=\"handleSeatClick(event, '$id', '$label', '$hostname', '$port', '$status')\">
...
<head>
    ...
    <style>
        ...
        body, html { margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg); min-height: 100vh; overflow-y: auto; }

        .navbar { background: var(--nav-green); padding: 0 2rem; display: flex; align-items: center; height: 60px; color: white; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); position: sticky; top: 0; z-index: 1000; }
        .nav-back-btn { color: white; text-decoration: none; margin-right: 15px; font-size: 1.2rem; transition: transform 0.2s; }
        .nav-back-btn:hover { transform: scale(1.2); }

        .edit-sidebar { position: fixed; right: 20px; top: 80px; background: white; padding: 15px; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 1px solid var(--border); z-index: 100; text-align: center; width: 140px; }
        ...
        .search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #94a3b8; z-index: 5; }
        #searchInput { width: 300px; padding: 10px 15px 10px 40px; border-radius: 12px; border: 1px solid var(--border); outline: none; background: white; font-family: inherit; transition: all 0.3s; box-shadow: var(--shadow-soft); }
        #searchInput:focus { width: 330px; border-color: var(--primary); box-shadow: 0 0 0 4px rgba(34, 197, 94, 0.1); }

        .main-container { padding: 20px 40px 100px 40px; display: flex; flex-direction: column; align-items: center; min-height: calc(100vh - 60px); }
        
        /* Effect: Glassmorphism Floor Plan */
        .floor-plan-wrapper { 
        ...
        .double-column-group { display: flex; gap: 0; border: 1px solid #f1f5f9; border-radius: 12px; overflow: hidden; background: white; }

        .seat-box { background: white; border: 1.5px solid var(--border); border-radius: 10px; padding: 5px; height: var(--seat-h); width: var(--seat-w); display: flex; flex-direction: column; justify-content: center; text-align: center; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; box-sizing: border-box; }
        
        /* Effect: Pulse animation for Vacant */
        @keyframes pulse-vacant {
            0% { box-shadow: 0 0 0 0 rgba(203, 213, 225, 0.4); }
            70% { box-shadow: 0 0 0 8px rgba(203, 213, 225, 0); }
            100% { box-shadow: 0 0 0 0 rgba(203, 213, 225, 0); }
        }
        .seat-box.Vacant { animation: pulse-vacant 2s infinite; }
        .seat-box.Occupied { background: var(--occupied-bg); border-color: var(--occupied-border); animation: none; }
        
        .seat-box:hover { transform: translateY(-4px) scale(1.02); z-index: 10; box-shadow: 0 12px 20px -5px rgba(0,0,0,0.1); border-color: var(--primary); }
        ...
        /* Effect: Edit/Swap Mode Styles */
        .edit-mode-active .seat-box { cursor: grab !important; border: 2px dashed var(--primary-orange) !important; animation: bounce 1s infinite; }
        @keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-2px); } }
        .drag-over { background: #fff7ed !important; border: 2px solid var(--primary-orange) !important; transform: scale(1.1) !important; z-index: 20; }
        
        /* Effect: Search Dimming */
        .dimmed { opacity: 0.1; filter: grayscale(1) blur(1px); pointer-events: none; transform: scale(0.9); }
        ...
        .status-bar { position: fixed; bottom: 15px; left: 50%; transform: translateX(-50%); display: flex; gap: 25px; background: white; padding: 10px 30px; border-radius: 50px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); font-size: 0.8rem; z-index: 1000; border: 1px solid var(--border); }
        
        .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(15, 23, 42, 0.4); backdrop-filter: blur(8px); z-index: 2000; animation: fadeIn 0.3s; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .modal-content { background: white; width: 340px; padding: 2rem; border-radius: 24px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); }
...
<div class="main-container">
    ...
    <div class="status-bar">
        <div style="display:flex; align-items:center; gap:8px;"><div style="width:10px; height:10px; border-radius:50%; background:var(--primary); box-shadow: 0 0 8px var(--primary);"></div> Occupied: <b><?php echo $occupiedCount; ?></b></div>
