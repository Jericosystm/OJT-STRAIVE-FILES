...
if(isset($_POST['update_seat'])) {
    ...
    // Check muna kung nage-exist na yung ID sa database
    ...
    if($res->num_rows > 0) {
        ...
        // Kung wala pa sa DB, i-INSERT (Para pumasok yung bagong data)
...
while($row = $result->fetch_assoc()) {
    $stations[$row['id']] = $row; // Map by ID para accurate
    ...
    <div class="map-grid-container">
        <div class="map-grid">
            ...
            for($i = 0; $i < $total_cells; $i++): 
                ...
                if ($cubicle_counter == 29 && !$pillar_placed) {
                ...
                // Database mapping - Ginagamitan ng global-like ID (Halimbawa: 2000 + counter para sa Toronto)
                $db_id = 2000 + $cubicle_counter; 
                $row = $stations[$db_id] ?? null;
            ...
            ?>
                <div class="seat-box <?php echo $status; ?>" 
                     ...
                     onclick="handleSeatClick(event, '<?php echo $db_id; ?>', '<?php echo $cubicle_name; ?>', '<?php echo $hostname; ?>', '<?php echo $port; ?>', '<?php echo $status; ?>')">
