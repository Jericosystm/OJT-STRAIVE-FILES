...
$page_title = "DENVER DEPARTMENT";
$back_link = "prod_map.php";

$department_name = "Denver"; 
$total_seats = 42; 
...
if(isset($_POST['swap_seats'])) {
    ...
    try {
        ...
        if (!empty($srcHost)) {
            // Get Target's Port for the Source Host
        ...
        if (!empty($tgtHost)) {
            // Get Source's Port for the Target Host
...
<head>
    ...
    <style>
        ...
        .header-row h1 { font-weight: 800; font-size: 1.8rem; letter-spacing: -0.03em; margin: 0; background: linear-gradient(to right, var(--text-dark), #64748b); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .map-grid-container { background: var(--card-bg); padding: 1.5rem; border-radius: 30px; border: 1px solid var(--border); flex-grow: 1; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-soft); overflow: hidden; }

        .map-grid { display: grid; grid-template-columns: repeat(6, 1fr); grid-template-rows: 1fr 1fr 25px 1fr 1fr 25px 1fr 1fr 25px 1fr; gap: 12px; width: 100%; height: 100%; }

        .walkway { grid-column: span 6; background: transparent; pointer-events: none; }

        .seat-box { border-radius: 16px; text-align: center; border: 1px solid transparent; background: var(--card-bg); color: var(--text-dark); transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; box-shadow: var(--shadow-soft); }

        .seat-box:hover { transform: translateY(-5px) scale(1.03); box-shadow: var(--shadow-hover); z-index: 10; border-color: #fbbf24; }
        ...
        /* --- EDIT SIDEBAR STYLES --- */
        .edit-sidebar { position: fixed; right: 20px; top: 90px; background: var(--card-bg); color: var(--text-dark); padding: 20px; border-radius: 20px; box-shadow: var(--shadow-hover); border: 1px solid var(--border); z-index: 100; display: flex; flex-direction: column; gap: 10px; transition: all 0.3s ease; }
        ...
        .status-legend { display: flex; justify-content: center; gap: 30px; margin-top: 1rem; background: var(--card-bg); color: var(--text-dark); padding: 0.8rem 2rem; border-radius: 50px; width: fit-content; margin-left: auto; margin-right: auto; box-shadow: var(--shadow-soft); border: 1px solid var(--border); }
        ...
        .modal-content { background: var(--card-bg); color: var(--text-dark); width: 420px; padding: 2.5rem; border-radius: 32px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); box-shadow: 0 30px 60px -12px rgba(0,0,0,0.3); border: 1px solid var(--border); }
    </style>
</head>
<body id="body">
...
<div class="container">
    <div class="header-row">
        <div><h1>Denver Floor Plan</h1></div>
    ...
    <div class="map-grid-container">
        <div class="map-grid" id="mapGrid">
            ...
            for($i = 0; $i < $total_seats; $i++): 
                ...
                $id = $row['id'] ?? ($i + 1); 
                $cubicle_num = $i + 1;
                $cubicle_name = "DEN-" . str_pad($cubicle_num, 4, '0', STR_PAD_LEFT);
            ...
            ?>
                ...
                <?php 
                if ($cubicle_num % 12 == 0 && $cubicle_num < $total_seats) {
    ...
    <div class="status-legend">
        <div class="legend-item">
            ...
            Occupied: <span style="color: #22c55e;"><?php echo $occupied_count; ?></span>
        </div>
        <div class="legend-item">
            <div class="dot" style="background: #cbd5e1;"></div>
            Vacant: <span style="color: #94a3b8;"><?php echo $vacant_count; ?></span>
        </div>
...
<script>
    ...
    function performSwap(src, tgt) {
        ...
        fetch('denver.php', { method: 'POST', body: formData })
