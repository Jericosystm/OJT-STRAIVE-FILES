...
<head>
    ...
    <style>
        :root {
            /* Default Dark Mode (From prod_map) */
            --primary: #ff6600;
        ...
        .header-section h4 { color: var(--primary); text-transform: uppercase; letter-spacing: 2px; font-size: 0.8rem; margin-bottom: 10px; font-weight: 800; }
        .header-section h1 { font-size: 2.8rem; margin: 0; font-weight: 800; letter-spacing: -2px; }

        .tab-wrapper { display: flex; gap: 15px; margin: 25px 0 40px 0; }
        .tab-btn {
            background: var(--card-bg); color: var(--text-main); text-decoration: none; padding: 12px 25px;
            border-radius: 12px; font-weight: 600; border: 1px solid var(--border);
            transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1);
            backdrop-filter: blur(10px);
        }
        .tab-btn.active { background: var(--primary); color: white; border-color: var(--primary); box-shadow: 0 10px 20px var(--primary-glow); }
        .tab-btn:hover:not(.active) { background: var(--card-hover); transform: translateY(-2px); }

        .header-flex { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
        
        .search-box form { 
            display: flex; background: var(--card-bg); border-radius: 15px; padding: 5px; 
            border: 1px solid var(--border); align-items: center; backdrop-filter: blur(10px);
            transition: all 0.3s ease;
        }
        .search-box form:focus-within { border-color: var(--primary); box-shadow: 0 0 15px var(--primary-glow); }
        .search-box input { background: transparent; border: none; color: var(--text-main); padding: 10px 15px; outline: none; width: 300px; font-family: inherit; }
        
        .btn-add { 
            background: var(--primary); color: white; border: none; padding: 12px 25px; 
            border-radius: 12px; cursor: pointer; font-weight: 800; transition: 0.3s;
        }
        .btn-add:hover { transform: translateY(-2px); box-shadow: 0 5px 15px var(--primary-glow); }

        .data-table { width: 100%; border-collapse: separate; border-spacing: 0 12px; }
        .data-table th { text-align: left; color: var(--text-muted); padding: 0 20px; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; }
        .data-table tr { background: var(--card-bg); transition: all 0.3s ease; }
        .data-table tr:hover { background: var(--card-hover); transform: scale(1.005); }
        .data-table td { padding: 20px; border-top: 1px solid var(--border); border-bottom: 1px solid var(--border); backdrop-filter: blur(5px); }
        .data-table td:first-child { border-radius: 20px 0 0 20px; border-left: 1px solid var(--border); }
        .data-table td:last-child { border-radius: 0 20px 20px 0; border-right: 1px solid var(--border); text-align: center; }
        ...
        .loc-badge { background: rgba(128,128,128,0.1); color: var(--text-muted); padding: 5px 12px; border-radius: 8px; font-size: 0.7rem; font-weight: 800; border: 1px solid var(--border); }
        .loc-badge.active-loc { border-color: var(--neon-green); color: var(--neon-green); background: rgba(0, 255, 153, 0.1); }
        .loc-badge.inactive-loc { border-color: #ef4444; color: #ef4444; background: rgba(239, 68, 68, 0.1); }
        
        .node-icon { background: var(--input-bg); width: 45px; height: 45px; border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-right: 15px; border: 1px solid var(--border); float: left; color: var(--primary); }

        .modal { display: none; position: fixed; z-index: 2000; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(15px); align-items: center; justify-content: center; }
        .modal-content { 
            background: var(--modal-bg); border: 1px solid var(--border); padding: 40px; width: 90%; 
            max-width: 500px; border-radius: 30px; max-height: 90vh; overflow-y: auto; color: var(--text-main);
            box-shadow: 0 50px 100px rgba(0,0,0,0.3);
        }
        .form-control { background: var(--bg); border: 1px solid var(--border); color: var(--text-main); width: 100%; padding: 12px; border-radius: 12px; margin: 10px 0 20px 0; box-sizing: border-box; font-family: inherit; }
        .sub-text { color: var(--text-muted); font-size: 0.75rem; display: block; text-transform: uppercase; font-weight: 800; letter-spacing: 1px; }
        .action-btn { background: var(--card-bg); border: 1px solid var(--border); color: var(--text-main); padding: 10px; border-radius: 10px; cursor: pointer; transition: 0.3s; }
...
<body>
        ...
        <div class="header-flex">
            <div class="search-box">
                <form action="" method="GET">
                    ...
                    <i class="fa-solid fa-magnifying-glass" style="margin-left: 15px; color: var(--primary);"></i>
                    <input type="text" name="search" placeholder="Query hostname or serial..." value="<?php echo htmlspecialchars($search_query); ?>">
                    
                    <?php if (!empty($search_query)): ?>
                        <a href="?tab=<?php echo $current_tab; ?>" style="color: var(--text-muted); margin-right: 10px;">
            ...
            <div style="display:flex; gap:10px;">
                <button onclick="exportData()" class="action-btn" style="padding: 12px 20px; border-color: var(--neon-green); color: var(--neon-green); font-weight: 800;">
                ...
                <?php if ($result->num_rows > 0): ?>
                    ...
                    <tr>
                        <td>
                            ...
                            <div>
                                <span style="font-weight:800; display:block;"><?php echo htmlspecialchars($item['asset_name']); ?></span>
                                <span class="sub-text"><?php echo htmlspecialchars($item['device_type']); ?></span>
                            </div>
                        </td>
                        <td>
                            <span class="mono-text" style="display:block;"><?php echo htmlspecialchars($item['hostname'] ?? 'NO-HOST'); ?></span>
                            <span class="sub-text" style="color:var(--primary)">SN: <?php echo htmlspecialchars($item['serial_num']); ?></span>
                        </td>
                        <td>
                        ...
                        <td>
                            <div><span class="status-dot"></span><span style="font-size:0.8rem; font-weight:800;"><?php echo strtoupper($item['status']); ?></span></div>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="5" style="text-align:center; padding:50px; color:var(--text-muted); font-weight:600;">NO ACTIVE NODES DETECTED.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </main>
    ...
    <div id="assetModal" class="modal">
        <div class="modal-content">
            <h2 id="modalTitle" style="color:var(--primary); margin: 0 0 2rem 0; font-weight: 800; letter-spacing: -1px;">Node Initialization</h2>
            <form id="assetForm">
                ...
                <div style="display:flex; gap:15px;">
                    <div style="flex:1">
                        ...
                        <input type="text" id="hostName" class="form-control mono-text" required>
                    </div>
                    <div style="flex:1">
                        <label class="sub-text">Serial Num</label>
                        <input type="text" id="serialNum" class="form-control mono-text" required>
                    </div>
                </div>
                ...
                <div id="onsiteInfo" style="display:none; background:rgba(128,128,128,0.05); padding:20px; border-radius:15px; border:1px solid var(--border); margin-bottom:15px;">
                ...
                <div id="agentInfo" style="display:none; background:rgba(128,128,128,0.05); padding:20px; border-radius:15px; border:1px solid var(--border); margin-bottom:15px;">
                    <label class="sub-text">Agent Details</label>
                    <input type="text" id="agentName" class="form-control" placeholder="Full Name">
                    <input type="email" id="agentEmail" class="form-control" placeholder="Email Address">
                ...
                <button type="submit" id="saveBtn" class="btn-add" style="width:100%;">APPLY CONFIGURATION</button>
    ...
    async function editAsset(item) {
        ...
        setVal('agent_email', item.agent_email);
    ...
    function exportData() {
        ...
        window.location.href = `export_assets.php?tab=${tab}&search=${encodeURIComponent(search)}`;
    }

    // Support for Escape key to close modal
