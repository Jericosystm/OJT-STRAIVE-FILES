...
// Kinuha natin lahat ng Indiana records
...
<head>
    ...
    <style>
        ...
        .nav-back-btn { color: #2e7d32; text-decoration: none; font-size: 1.5rem; display: flex; align-items: center; transition: transform 0.2s; }
        .nav-back-btn:hover { transform: scale(1.1); }
        .nav-title { color: #2e7d32; font-weight: 900; font-size: 1.4rem; letter-spacing: -0.02em; }
        ...
        .map-grid-container { background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(10px); padding: 1.5rem; border-radius: 30px; border: 1px solid rgba(255,255,255,0.8); flex-grow: 1; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-soft); overflow-y: auto; }
        
        /* PINALITAN KO LANG ITO PARA SUMAKTO SA DATA FETCHING PERO ANG STYLE AY PAREHAS PA RIN */
        ...
        .drag-over { background: #f0fff0 !important; transform: scale(1.1) !important; border: 2px solid var(--primary-dark) !important; }
...
<nav class="navbar">
    ...
    <div class="nav-title">OJTBox | Indiana Dashboard</div>
</nav>
        ...
        <div class="map-grid" id="mapGrid">
            ...
            for($row_idx = 0; $row_idx < 9; $row_idx++): 
                for($col_idx = 0; $col_idx < 5; $col_idx++):
                    ...
                    // Layout logic (Huwag galawin para hindi masira ang alignment)
                    if($col_idx == 0 && $row_idx > 0) { echo '<div class="empty-slot"></div>'; continue; }
                    if($cubicle_pointer > 37) { echo '<div class="empty-slot"></div>'; continue; }

                    // DATA FETCHING: Itinatapat natin yung record sa database sa tamang box pointer
                    $db_row = isset($stations[$cubicle_pointer - 1]) ? $stations[$cubicle_pointer - 1] : null;
                    
                    $id = $db_row['id'] ?? 0; 
                    $cubicle_name = "IND-" . str_pad($cubicle_pointer, 4, '0', STR_PAD_LEFT);
                    $status = $db_row['status'] ?? 'Vacant';
                    $hostname = $db_row['hostname'] ?? '';
                    $switch_port = $db_row['campaign'] ?? 'Not Set';
            ?>
                    <div class="seat-box <?php echo $status; ?>" 
                        ...
                        <div style="font-size: 0.7rem; font-weight: 700; margin-top:2px;">
                            <?php echo ($status === 'Occupied' && !empty($hostname)) ? $hostname : 'Available'; ?>
                        </div>
                    </div>
            <?php 
                    $cubicle_pointer++;
    ...
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <div style="display: flex; gap: 15px; margin-top: 1rem;">
                <button type="submit" name="update_seat" style="flex:2; padding: 1rem; background: #2e7d32; color: #fff; border:none; border-radius:14px; font-weight:800; cursor:pointer;">UPDATE</button>
...
<script>
    ...
    function handleDrop(e) {
        ...
        if (sourceId && targetId && sourceId !== targetId) {
            ...
            fetch('indiana.php', { method: 'POST', body: formData }).then(r => r.json()).then(d => { if(d.success) location.reload(); });
        }
    }
