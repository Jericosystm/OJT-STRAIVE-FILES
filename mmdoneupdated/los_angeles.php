...
if(isset($_POST['swap_seats'])) {
    ...
    // Use 'campaign' or 'switch_port' based on your DB schema (matching your Dallas fix)
    ...
    // Swap data but force department to stay Los Angeles
    $updateSource = $conn->prepare("UPDATE production_floor_map SET hostname=?, status=?, campaign=?, department=? WHERE id=?");
    $updateSource->bind_param("ssssi", $targetData['hostname'], $targetData['status'], $targetData['campaign'], $department_name, $sourceId);
    
    $updateTarget = $conn->prepare("UPDATE production_floor_map SET hostname=?, status=?, campaign=?, department=? WHERE id=?");
    $updateTarget->bind_param("ssssi", $sourceData['hostname'], $sourceData['status'], $sourceData['campaign'], $department_name, $targetId);
...
<script>
    ...
    // Check localStorage on page load to see if Swap Mode was previously ON
