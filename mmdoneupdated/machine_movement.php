...
<head>
    ...
    <style>
        ...
        .tab-wrapper { display: flex; gap: 10px; background: #151515; padding: 5px; border-radius: 10px; border: 1px solid var(--border-color); }
        .tab-link { 
        ...
        .search-input {
            width: 100%; background: #151515; border: 1px solid var(--border-color);
        ...
        .data-table tbody tr:hover { transform: translateX(8px); background: #1a1a1a; box-shadow: -4px 0 0 var(--neon-blue); }
        .data-table td { padding: 18px 20px; border-top: 1px solid var(--border-color); border-bottom: 1px solid var(--border-color); }
        ...
        .badge-return { color: #facc15; border-color: rgba(250, 204, 21, 0.3); background: rgba(250, 204, 21, 0.1); }
        .badge-release { color: var(--neon-green); border-color: rgba(0, 255, 153, 0.3); background: rgba(0, 255, 153, 0.1); }
        ...
        .btn-edit { background: none; border: 1px solid #333; color: var(--neon-blue); padding: 6px 12px; border-radius: 5px; cursor: pointer; font-size: 0.8rem; }
        ...
        .modal-content { 
            background: #111; border: 1px solid #333; padding: 30px; width: 95%; max-width: 500px; border-radius: 15px; 
            box-shadow: 0 0 30px rgba(0,0,0,0.5); max-height: 90vh; overflow-y: auto;
        }
        .form-group label { display: block; color: var(--text-gray); font-size: 0.75rem; text-transform: uppercase; margin-bottom: 8px; letter-spacing: 1px; }
        .form-group input, .form-group select, .form-group textarea { 
            width: 100%; background: #1a1a1a; border: 1px solid #333; color: white; padding: 12px; border-radius: 8px; box-sizing: border-box; margin-bottom: 15px;
        }
        .btn-save { background: var(--neon-green); color: black; border: none; padding: 12px; width: 100%; border-radius: 8px; font-weight: bold; cursor: pointer; }
...
<body>
    ...
    <main class="log-container">
        <div class="header-flex">
            <div>
                ...
                <h1 style="margin:5px 0 0 0; font-size:2rem;">Machine Movement</h1>
            </div>
        ...
        <table class="data-table" id="movementTable">
            ...
            <tbody>
                <?php if ($result->num_rows > 0): ?>
                    ...
                    <tr class="log-row">
                        <td>
                            <span style="font-weight:700; display:block; color:white;"><?php echo htmlspecialchars($row['asset_name']); ?></span>
                            <span style="font-size:0.75rem; color:var(--text-gray);"><?php echo htmlspecialchars($row['device_type']); ?></span>
                        </td>
                        <td>
                            <code style="color:var(--primary-orange);"><?php echo htmlspecialchars($row['hostname']); ?></code>
                            <div style="font-size:0.7rem; color:#555;">SN: <?php echo htmlspecialchars($row['serial_number']); ?></div>
                        </td>
                        <td>
                            <?php if(!empty($row['agent_name'])): ?>
                                <div style="font-weight:600; color:var(--neon-blue); font-size:0.85rem;"><?php echo htmlspecialchars($row['agent_name']); ?></div>
                                <div style="font-size:0.7rem; color:var(--text-gray);"><?php echo htmlspecialchars($row['agent_email']); ?></div>
                                <div style="font-size:0.65rem; color:#555;">Sup: <?php echo htmlspecialchars($row['immediate_supmail']); ?></div>
                            <?php else: ?>
                                <span style="color:#333; font-style:italic; font-size:0.75rem;">N/A</span>
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td style="font-size:0.85rem; font-weight:600;"><?php echo htmlspecialchars($row['moved_by']); ?></td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const forms = ['manualLogForm', 'editLogForm'];
        forms.forEach(id => {
