...
<head>
    ...
    <style>
        ...
        .select-input option, .form-group select option {
        ...
        .form-group input, .form-group select { width: 100%; padding: 12px; background: #111; border: 1px solid var(--border); color: #fff; border-radius: 12px; outline: none; }
...
<body>
    ...
    <main class="inventory-container">
        <div class="header-flex">
            <div>
                <p style="color: var(--primary); font-weight: 800; font-size: 0.75rem; letter-spacing: 5px; text-transform: uppercase; margin-bottom: 10px;">End User Computing Schedules</p>
            ...
            <table class="data-table">
                ...
                <tr>
                    ...
                    <?php foreach ($week_days as $day): 
                        ...
                        $isDayOff  = (strcasecmp($stype, 'Day Off') == 0);
                    ?>
                        <td class="<?= $is_admin ? 'clickable-cell' : '' ?>" 
                            ...
                            <?php elseif($isDayOff): ?>
                                <span class="rest-day-badge" style="border-color: var(--danger); color: var(--danger);">Day Off</span>
    ...
    <div id="assignModal" class="modal">
        <div class="modal-content">
            ...
            <form id="schedForm" action="save_schedule.php" method="POST">
                ...
                <div class="checkbox-group" style="margin-top: 20px; padding: 12px; background: rgba(255,102,0,0.05); border-radius: 12px; border: 1px solid var(--border); display: flex; flex-direction: column; gap: 12px;">
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 0.85rem;">
                        ...
                        <span>Apply to all <strong id="dynamic_day_name" style="color: var(--primary);">Mondays</strong> this month</span>
                    </label>

                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 0.85rem;">
                        <input type="checkbox" name="apply_to_entire_month" id="apply_to_entire_month" style="width: 18px; height: 18px; accent-color: var(--primary);">
                        <span>Apply to the <strong style="color: var(--primary);">Entire Month</strong> for this user</span>
                    </label>
                </div>
    ...
    function openModal(techId, date, type, tin, tout, hasRecord) {
        ...
        document.getElementById('apply_to_entire_month').checked = false;
    ...
    function deleteSchedule() {
        if(confirm("Delete schedule record(s)?")) {
    ...
    function toggleTimeFields() {
        ...
        const isNotTimed = (type === 'Rest Day' || type === 'Day Off');
        document.getElementById('time_fields').style.display = isNotTimed ? 'none' : 'block';
    }
