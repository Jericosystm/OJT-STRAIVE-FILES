...
if (isset($_GET['delete_id'])) {
    ...
    // Fetch filename first for the log
    $stmt = $conn->prepare("SELECT file_name, file_path FROM hdn_files WHERE id = ?");
    ...
    if ($file = $res->fetch_assoc()) {
        ...
        if ($del_stmt->execute()) {
            // --- TRACKER LOGIC ---
            $log_action = "DELETE";
            $log_details = "Permanently deleted document: " . $fname;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdf_file'])) {
    ...
    if (strtolower(pathinfo($target_file, PATHINFO_EXTENSION)) == "pdf") {
        if (move_uploaded_file($_FILES["pdf_file"]["tmp_name"], $target_file)) {
            ...
            if ($stmt->execute()) {
                // --- TRACKER LOGIC ---
                $log_action = "INSERT"; // This will trigger your green badge
                $log_details = "Uploaded new HDN document: " . $file_name;
                $log_stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, created_at) VALUES (?, ?, ?, NOW())");
                $log_stmt->bind_param("iss", $_SESSION['user_id'], $log_action, $log_details);
                $log_stmt->execute();
            }
