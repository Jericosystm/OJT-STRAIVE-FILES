...
if ($_POST['ajax_action'] == 'update') {
        ...
        // 1. Fetch hostname to make the log more descriptive
        $res = $conn->query("SELECT hostname FROM win_baseline WHERE id = $id");
        $host_data = $res->fetch_assoc();
        $h_name = $host_data['hostname'] ?? "Unknown Host";

        // 2. Determine Log Formatting
        $col_display = strtoupper(str_replace('_', ' ', $column));
        
        if ($value == 'DONE') {
            $action_type = "BASELINE_DONE";
            $details = "Completed requirement [$col_display] for Host: $h_name";
        } else {
            $action_type = "BASELINE_UPDATE";
            $details = "Updated [$col_display] for Host: $h_name to '$value'";
        }

        // 3. Record the activity
        recordActivity($conn, $u_id, $action_type, $details);
  ...
  if ($_POST['ajax_action'] == 'delete_row') {
    ...
    // 1. Fetch the hostname before deleting the record
    $res = $conn->query("SELECT hostname FROM win_baseline WHERE id = $id");
    if ($res && $res->num_rows > 0) {
        $host_data = $res->fetch_assoc();
        $h_name = $host_data['hostname'];
        
        // 2. Record the activity with the specific hostname
        recordActivity($conn, $u_id, "BASELINE_ROW_DELETE", "Deleted, Hostname: $h_name (ID: $id)");
    } else {
